...
@Component({
  selector: 'app-qcalendarmldg',
  templateUrl: './qcalendarmldg.component.html',
  styleUrls: ['./qcalendarmldg.component.css'],
  providers: [DatePipe]
})
export class QcalendarmldgComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.plantservice
      ...
      .then(res => {
        ...
        if (res) {
          me.dpservice
            .New_getRejectcalendar(me.selectedcode, "ZMLD", me.startdate)
            .toPromise()
            .then(res => {
  ...
  loaddata() {
    const me = this;
    this.dpservice
      .New_getRejectcalendar(this.selectedcode, "ZMLD", this.startdate)
      .toPromise()
      .then(res => {
        this.dpservice.dailyprodlist = res as Dailyproduction[];
        this.countRejrecord();
  ...
  loadchart1() {
    ...
    this.dpservice.New_getprochartsummary(this.selectedcode, "ZMLD", monthName, this.startdate).then((res: any) => {
      this.dpservice.dailyreportsummary.forEach(drsummary => {
        if (drsummary.itemtype === "ZMLD") {
  ...
  selectedGrid(ev) {
    ...
    this.dpservice
      .New_getRejectcalendar(ev, "ZMLD", this.startdate)
      .toPromise()
      .then(res => {
        this.dpservice.dailyprodlist = res as Dailyproduction[];
        this.countRejrecord();
  ...
  extraVal(val) {
    ...
    this.loading = true;
    this.dpservice.New_getRejectdetail(this.selectedcode, "ZMLD", firstDate, endDate)
      .toPromise()
      .then(res => {
        this.dpservice.itemwiserejlist = res as Itemwiserej[];
        this.sumAllData();
